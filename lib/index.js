...
//        const now = new Date().getTime();
        ...
        router.use("apidocs", module.context.createDocumentationRouter())
        .summary("Browseable API Documentation")
        ...
        switch(options.model) {
            case "graph":
                if (!db._collection(qName)) {
                    var coll = db._createEdgeCollection(qName);
                    console.log("created graph: %s.", qName);
                    if (options.example) {
                        coll.insert(options.example);
            ...
            default:
                if (!db._collection(qName)) {
                    var graph = db._createDocumentCollection(qName);
                    console.log("created collection: %s", qName);
                    if (options.example) {
                        graph.insert(options.example);
                        console.log("example: %s created.", qName);
                    }
                } else {
                    console.log("collection %s found.", qName);
                }
                break;
        }
