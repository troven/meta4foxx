...
//        const now = new Date().getTime();
        ...
        DEBUG && console.log("Module: depends on: %o", _.keys(module.context.dependencies));
        ...
        return { options: options, router: router, apis: {},
            ...
            api: function(defn) {
                ...
                DEBUG && console.log("Registered [%s] API: %o -> %o", defn.router?"Existing":"New", api.name, _.keys(api));
                return defn;
            }
        };
    },
    ...
    API: function(_defaults, _options) {
        ...
        DEBUG && console.log("API: %s @ %s -> ", options.name, options.manifest.name, strategy);
        ...
        DEBUG && console.log("API Strategy: %s => %o => %o", options.name, strategy, _.keys(api));
        return api;
    },
    ...
    Setup: function(_defaults, _options) {
        ...
        console.log("initial data: %o.", examples);
        ...
        if (options.uniques) {
            ...
            console.log("[%s] %s x uniques for %s", manifest.name, options.uniques.length, qName);
        }
        return models;
    },
