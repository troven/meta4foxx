...
//        DEBUG && console.log("[meta4foxx] RenderReport: %s -> %s", req.path, req.headers);
        var contentType = req.headers.accept;
        if (req.path && req.path.endsWith(".csv")) contentType = "text/csv";
    ...
    EncryptModel : function(model, options) {
        ...
        DEBUG & console.log("[meta4foxx] Encrypting %o", model);
