...
module.exports = function (options, mixins) {
    ...
    DEBUG && console.log("Upload API: %s", options.name);

    // UPLOAD | CREATE

    if (can.upload || can.post) {
        var endpoint = router.post(path, rbac.protect(function (req, res) {
            DEBUG & console.log("API upload: %o -> %o", req.path, _.keys(req));
...
//                DEBUG &&
                ...
                //DEBUG &&
                DEBUG && console.log("API get-all: %o -> %o found", req.path, util.models.length);
                res.send(models);
            }
        }));
        ...
        endpoint = router.get(path + "/:_key", rbac.protect(function (req, res) {
            ...
            util.models.DecryptModel(model);
    ...
    //DEBUG &&
    DEBUG && console.log("[%o] Upload: %o", manifest.name, options.singular);
