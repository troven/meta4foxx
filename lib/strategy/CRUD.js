...
module.exports = function (options, mixins) {
    ...
    console.log("CRUD collection: %s -> %o ->%o", qname, _.keys(collection), _.keys( collection.all() ) );
    ...
    if (_.isObject(options.schema) && (options.schema.properties || options.schema.items) ) {
        ...
        DEBUG && console.log("JSON-Schema for %s -> %o", options.name, options.schema);
...
>>>>>>> 5cd6f16c30da93a9baa659ca405ff02ebdda14d5
    DEBUG && console.log("[%s] %s scoped: %o", options.manifest.name, options.path, options.scoped);
    var rbac = RBAC(options.scoped, options);

    DEBUG && console.log("CRUD API: %s", options.name);

    // GET META-DATA

    DEBUG && console.log("meta-data: %s -> %s -> %o -> %o", path, can.get, options.defaults, options.example);
    ...
    if (can.get) {
        ...
        var endpoint = router.get(path, rbac.protect(function (req, res) {
            ...
            DEBUG & console.log("RBAC %s permit: %o -> %o", req.path, permit.can("get", req.path));
            ...
            if (options.queryParams===true || options.byExample) {
                ...
                DEBUG && console.log("byExample1: %o -> %o -> %o", _.keys(options), filter, req.queryParams);
                ...
                DEBUG && console.log("byExample2: %o -> %o", _.keys(options), filter);
            ...
            if (filter) {
                ...
                DEBUG && console.log("API get-by: %s -> %o -> %o", req.path, filter, models);
            } else {
                ...
                DEBUG && console.log("API get-all: %o -> %o found", req.path, models.length);
            }

            _.each(models, function(model) {
                console.log("CRUD model: %o", model);
            });
        ...
        endpoint = router.get(path + "/:_key", rbac.protect(function (req, res) {
            ...
            //DEBUG &&
            DEBUG && console.log("API get: %o -> %o", filter, model);
    ...
    if (can.put) {
        var endpoint = router.put(path, rbac.protect(function (req, res) {
            ...
            //DEBUG &&
            DEBUG && console.log("API update: %o -> %o", req.path, model);
    ...
    //DEBUG &&
    DEBUG && console.log("[%o] CRUD: %o", manifest.name, options.singular);
