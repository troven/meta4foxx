...
module.exports = function (options, mixins) {
    ...
    console.log("[meta4foxx] CRUD collection: %s -> %o ->%o", name, qname, collection?"found-collection":"collection-missing:"+qname );
    assert(collection, "Missing collection: "+qname+" for "+name);
    ...
    if (_.isObject(options.schema) && (options.schema.properties || options.schema.items) ) {
        ...
        options.examples = options.examples || [ options.example];

        // Validate example against schema :-)
        if (!_.isEmpty(options.examples) && _.isArray(options.examples)) {
            _.each(options.examples, function(example) {
                var valid = schema.validate(options.example);
                DEBUG && console.log("[meta4foxx] Validate JSON-Schema for %s ==> %s", options.name, valid.error?false:true);
                if (valid.error) {
                    console.log("Example Errors: %o ---> %o", options.example, valid.error);
                    throw new Error("Example data for "+options.name+" API does not match schema", options.example);
                }
            });
        }
//        schema.object().string("_key");
    ...
    if (can.get) {
        ...
        var endpoint = router.get(path, rbac.protect(function (req, res) {
            ...
            if (_.isArray(options.queryParams)) {
                _.each(options.queryParams, function(qp) {
                    assert(_.isString(qp), "queryParams must be array() of strings");
                    if (req.queryParams[qp] && req.queryParams[qp]!="") filter[qp] = req.queryParams[qp] || "";
                });
                DEBUG && console.log("[meta4foxx] byExample2: %o -> %o", _.keys(options.queryParams), filter);
            } else if (options.queryParams===true || options.byExample) {
                ...
                throw new Exception("API queryParams is neither 'true' nor an array of string");
            }
        ...
        endpoint = router.get(path + "/:_key", rbac.protect(function (req, res) {
            ...
            DEBUG && console.log("[meta4foxx] API get: %o", filter);
