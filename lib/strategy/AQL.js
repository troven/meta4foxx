...
module.exports = function (options, mixins) {
    ...
    try {
        ...
        console.log("[meta4foxx] Report Definition Error: %o", e);
    ...
    DEBUG && console.log("[meta4foxx] AQL %s API: %s", options.name, options.manifest.name);
    ...
    _.each(reports_defn.reports, function(report) {
        ...
        DEBUG && console.log("[meta4foxx] AQL Report: %s / %s -> %o -> %o", path, report.name, report, query_file );

        var Renderer = rbac.protect(function (req, res) {
            ...
            try {
                ...
                console.log("[meta4foxx] FILE ERROR: %o -> %o", query_file, e);
                res.sendStatus(404);
            ...
            try {
                ...
                DEBUG && console.log("[meta4foxx] QUERY: %s [%o] <- %o -> %o", req.path, filter, query, result);
                mixins.afterRead && mixins.afterRead(result, req, res, options);

                util.models.RenderReport(result, req, res);
                // res.send(result);
            } catch(e) {
                console.log("[meta4foxx] QUERY ERROR: %o -> %o", query_file, e);
                res.sendStatus(500);
        ...
        var endpoint = router.get(path + "/"+ report.name, Renderer);
    ...
    //DEBUG &&
    DEBUG && console.log("[meta4foxx] [%o] AQL: %o -> %o", manifest.name, options.plural, _.keys(reports_defn.reports));
