...
module.exports = function(options, defaults, mixins) {
    ...
    assert(options.config, "Missing module config");
    assert(options.context, "Missing context");
    assert(options.context.service, "Missing service context");
    assert(options.context.service.manifest, "Missing service context");
    assert(options.router, "Missing service manifest");

    options.name  = options.name || "self";
    ...
    var THIN_JWT_KEY = config.jwt_keyspace || options.jwt_keyspace || "subject_token";
