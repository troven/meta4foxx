...
//    console.log("[meta4foxx] Custom collection: %s -> %o -> %o", qname, _.keys(collection), _.keys( collection.all() ) );
    ...
    _.each(options.paths, function(methods, path) {
        _.each(methods, function(defn, method) {
            ...
            DEBUG && console.log("[meta4foxx] [%o] Custom: %o", manifest.name, method, path);

            var endpoint = operation.apply(router, [path, function( req, res) {
                DEBUG && console.log("[meta4foxx] Triggered API: %s %s", method, path);
    ...
    //
    // DEBUG &&
    DEBUG && console.log("[meta4foxx] [%o] Custom: %o", manifest.name, options.singular);
