...
module.exports = function(scoped, options) {
    ...
    var CanScope = function(_verbs) {
        ...
        DEBUG && console.log("_verbs: %o", _verbs);
    ...
    DEBUG && console.log("VERBS: %o", verbs)
    CanScope(verbs);
    ...
    var RBAC = {
        ...
        authorize: function(jwt, req) {
            ...
            DEBUG && console.log("rbac jwt roles: %o", roles);
            ...
            DEBUG && console.log("rbac authorizes: %o --> %s --> %o", roles, scoped, matched);
            ...
            return {
                ...
                can: function(action, resource) {
                    ...
                    if (!jwt) {
                        DEBUG && console.log("rbac no-jwt: %s --> %o", action, resource);
                        return false;
                    }
                    var granted = matched.indexOf?matched.indexOf(action)>=0:matched[action]?true:false;
                    DEBUG && console.log("rbac granted?: %s --> %s --> %o", action, matched, granted);
                    return granted;
                }
